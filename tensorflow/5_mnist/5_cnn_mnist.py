...
def main():
    ...
    filter1 = tf.Variable(tf.truncated_normal([5, 5, 1, 32]), name="filter1")
    ...
    # #第二层卷积
    filter2 = tf.Variable(tf.truncated_normal([5, 5, 32, 64]), name="filter2")
    ...
    # # feedforward
    ffn_w = tf.Variable(tf.truncated_normal([7 * 7 * 64, 1024]))
    ffn_b = tf.Variable(tf.cast(tf.fill([1024], 0.1), tf.float32))
    max_pool_2_flat = tf.reshape(max_pool_2, [-1, 7 * 7 * 64])
    fnn_h = tf.nn.relu(tf.matmul(max_pool_2_flat, ffn_w) + ffn_b)

    # # softmax
    ffn2_w = tf.Variable(tf.truncated_normal([1024, 10], name="ffn2_w"))
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for i in range(2000):
            for batch_images, batch_labels in batch(train_images, train_labels, batchsize=1):
                loss_, acc_, _ = sess.run(
                    [loss, acc, train_op], feed_dict={X: batch_images, Y: batch_labels})
