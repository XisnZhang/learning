...
class TextRank:
    ...
    def analysis(self, text):
        ...
        self.keywords = self.tr4w.get_keywords(num=6, word_min_len=2)
        self.keyphrases = self.tr4w.get_keyphrases(keywords_num=6, min_occur_num=2)

    def key_sentence_extract(self, text):
        tr4s = TextRank4Sentence()
        tr4s.analyze(text, lower=True, source='all_filters')
        self.keysentences = tr4s.get_key_sentences(num=3, sentence_min_len=6)
        return self.keysentences
...
if __name__ == '__main__':
    ...
    start = time.time()
    print(tr.key_sentence_extract(text))
    print("cost time: ", time.time() - start)
    print("jieba textrank: ")
    start = time.time()
    print(textrank(text, topK=6))
    print("cost time: ", time.time() - start)
    print("snow nlp：")
    start = time.time()
    snlp = SnowNLP(text)
